...
const organText = document.getElementById('organ-placeholder-text');
...
function renderReport(data) {
    ...
    createCard('fas fa-weight-hanging', 'التقديرات الحيوية', `
        <div style="display:flex; gap:15px; flex-wrap: wrap;">
            ...
            <div style="flex:1; background:#f9f9f9; border-radius:10px; padding:12px; display:flex; align-items:center; gap:15px; border:1px solid #eee;">
                <div style="background:#e8eaf6; color:#3949ab; width:50px; height:50px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.4rem;">
                    <i class="fas fa-weight"></i>
                </div>
                <div>
                    <div style="font-weight:800; font-size:1.2rem; color:#333; margin-bottom:4px;">${data["2_weight_est"].value}</div>
                    <div style="font-size:0.9rem; color:#666; font-weight:bold;">الوزن التقديري</div>
                </div>
            </div>
            ...
            <div style="flex:1; background:#f9f9f9; border-radius:10px; padding:12px; display:flex; align-items:center; gap:15px; border:1px solid #eee;">
                ...
                <div>
                    <div style="font-weight:800; font-size:1.2rem; color:#333; margin-bottom:4px;">${data["3_age_est"].value}</div>
                    <div style="font-size:0.9rem; color:#666; font-weight:bold;">العمر التقديري</div>
                </div>
            </div>
    ...
    if (reasons.feces_color && reasons.feces_color !== 'N/A') {
        ...
        <div style="margin-top:15px; border-top:1px dashed #ccc; padding-top:15px;">
            <h6 style="color:#795548; margin-bottom:10px; font-weight:bold; display:flex; align-items:center; gap:5px;">
                <i class="fas fa-flask"></i> تحليل عينة البراز:
            </h6>
    ...
    if (reasons.organ_analysis && reasons.organ_analysis !== 'N/A') {
        organHtml = `
        <div style="margin-top:15px; border-top:1px dashed #ccc; padding-top:15px;">
            <h6 style="color:#c62828; margin-bottom:10px; font-weight:bold; display:flex; align-items:center; gap:5px;">
                <i class="fas fa-heartbeat"></i> الفحص التشريحي (${selectedOrganName}):
            </h6>
            <div style="color:#444; font-size:0.95rem; line-height:1.6; padding-right:10px;">
                ${reasons.organ_analysis}
            </div>
        </div>`;
    }
    ...
    const symptomsHtml = `
        <div style="margin-top:15px; background:#f8f9fa; border:1px solid #e9ecef; border-radius:8px; padding:15px;">
            <h5 style="color:#3949ab; margin-bottom:10px; font-weight:bold; display:flex; align-items:center; gap:5px;">
                <i class="fas fa-dove"></i> التحليل الظاهري (الدجاجة):
            </h5>
            
            <div style="padding-right:10px;">
                <div style="margin-bottom:8px; display:flex; gap:5px;">
                    <strong style="color:#333; min-width:90px;"> وضعية الرأس:</strong> 
                    <span style="color:#555;">${reasons.head}</span>
                </div>
                <div style="margin-bottom:8px; display:flex; gap:5px;">
                    <strong style="color:#333; min-width:90px;"> التوازن:</strong> 
                    <span style="color:#555;">${reasons.balance}</span>
                </div>
                <div style="margin-bottom:8px; display:flex; gap:5px;">
                    <strong style="color:#333; min-width:90px;"> الحركة:</strong> 
                    <span style="color:#555;">${reasons.movement}</span>
                </div>
                <div style="margin-bottom:8px; display:flex; gap:5px;">
                    <strong style="color:#333; min-width:90px;"> العين:</strong> 
                    <span style="color:#555;">${reasons.eyes}</span>
                </div>
                <div style="display:flex; gap:5px;">
                    <strong style="color:#333; min-width:90px;"> الريش:</strong> 
                    <span style="color:#555;">${reasons.feathers}</span>
                </div>
            </div>
    ...
    createCard('fas fa-user-md', 'التشخيص الأساسي', `
        <div style="padding: 5px;">
            ...
            <div style="margin-bottom: 15px;">
                <h3 style="color:#c62828; margin:0 0 5px 0; font-weight:900; font-size:1.6rem; line-height:1.2;">
                    ${data["4_primary_diagnosis"].disease_name_ar}
                </h3>
                <h4 style="color:#555; font-weight:bold; margin:0; font-family:sans-serif; font-size:1.1rem;">
                    ${data["4_primary_diagnosis"].disease_name_en}
                </h4>
            </div>
            
            <div style="display:flex; align-items:center; gap: 10px; margin-bottom: 20px;">
                <strong style="color:#333; font-size:1.1rem;">نسبة الاشتباه:</strong> 
                <span style="background:#ffebee; color:#c62828; padding:4px 15px; border-radius:6px; font-weight:900; font-size:1.3rem; border:1px solid #ffcdd2;">
                    ${data["4_primary_diagnosis"].probability}
                </span>
            </div>
    ...
    data["5_alternatives"].diseases.forEach(d => {
        ...
        <li style="background:#fff; border:1px solid #eee; border-radius:12px; padding:15px; margin-bottom:12px; box-shadow: 0 2px 5px rgba(0,0,0,0.02); transition: transform 0.2s;">
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                <div>
                    <div style="font-weight:900; font-size:1.1rem; color:#2c3e50;">${d.name_ar}</div>
                    <div style="font-size:0.85rem; color:#95a5a6; font-family:sans-serif;">${d.name_en}</div>
                </div>
                
                <span style="background:#e3f2fd; color:#1565c0; padding:5px 12px; border-radius:8px; font-weight:900; font-size:1rem; border:1px solid #bbdefb;">
                    ${d.prob}
                </span>
            </div>
            
            <div style="font-size:0.95rem; color:#555; margin-bottom:10px; padding-right:10px; border-right:3px solid #cfd8dc; line-height:1.5;">
                <strong style="color:#455a64;">السبب:</strong> ${d.reason}
            </div>

            <div style="text-align:left;">
                <a href="${d.link}" target="_blank" style="font-size:0.85rem; color:#3949ab; text-decoration:none; font-weight:bold; display:inline-flex; align-items:center; gap:5px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 12px;">
            ...
            <div style="background:#fff; border:1px solid #ffcdd2; border-radius:10px; padding:15px;">
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
                    <div style="background:#ffebee; color:#c62828; width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.1rem;">
                        <i class="fas fa-door-closed"></i>
                    </div>
                    <strong style="font-size:1.1rem; color:#c62828;">إجراءات العزل:</strong>
                </div>
            ...
            <div style="background:#fff; border:1px solid #bbdefb; border-radius:10px; padding:15px;">
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
                    <div style="background:#e3f2fd; color:#1565c0; width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.1rem;">
                        <i class="fas fa-utensils"></i>
                    </div>
                    <strong style="font-size:1.1rem; color:#1565c0;">التغذية والماء:</strong>
                </div>
            ...
            <div style="background:#fdf2ff; border:1px solid #e1bee7; border-radius:10px; padding:15px;">
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
                    <div style="background:#fff; color:#8e24aa; width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.1rem; border:1px solid #ba68c8;">
                        <i class="fas fa-prescription-bottle-alt"></i>
                    </div>
                    <strong style="font-size:1.1rem; color:#7b1fa2;">البروتوكول العلاجي (الدواء):</strong>
                </div>
            ...
            <div style="background:#fff; border:1px solid #c8e6c9; border-radius:10px; padding:15px;">
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
                    <div style="background:#e8f5e9; color:#2e7d32; width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.1rem;">
                        <i class="fas fa-temperature-high"></i>
                    </div>
                    <strong style="font-size:1.1rem; color:#2e7d32;">البيئة والحرارة:</strong>
                </div>
            ...
            <div style="background:#fff; border:1px solid #ffe0b2; border-radius:10px; padding:15px;">
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
                    <div style="background:#fff3e0; color:#ef6c00; width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.1rem;">
                        <i class="fas fa-vial"></i>
                    </div>
                    <strong style="font-size:1.1rem; color:#ef6c00;">الفحوصات المخبرية:</strong>
                </div>
